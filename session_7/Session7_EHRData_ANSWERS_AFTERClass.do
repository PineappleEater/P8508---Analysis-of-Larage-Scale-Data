...
merge 1:1 subject_id hadm_id using admissions
...
/*what kind of merge do you think we should do?*/
drop _merge
merge m:1 subject_id stay_id using triage
...
/*what kind of merge do you think we should do?*/
drop _merge
merge 1:m subject_id hadm_id using poe
...
/*what kind of merge do you think we should do?*/
drop _merge
merge 1:m subject_id   poe_id using poe_detail
keep if _merge==3 | _merge==1 /*keep our subject's records, regardless of whether they linked or not*/
...
gen arrival_date = date(intime, "YMD hms") /*you need to make sure you use the correct format. the raw data is YMD hms (year, month, day, hour minute second). If you opened and saved it in excel first, it might have changed to be MDY hm*/
...
duplicates drop stay_id ndc, force
 ...
 bysort stay_id  : generate ndc_id = _n /*this is just so we can number each NDC within the stay -- for example, if we wanted to see how many medications each person was on at the time of their ED visit*/
...
gen manufacuturer=substr(ndc, 1, 5)
...
/*stay ID is "hinge"
ndc_id will be the suffix/stem of the new column - this is just the variable that sequentially counts each ndc within a stay.
We use "wide" because we are going from long to wide.
*/
