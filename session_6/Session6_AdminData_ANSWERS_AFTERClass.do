...
/*YOU TRY: convert birthweight to a numeric field and call it birthweight_num*/

destring birthweight, gen(birthweight_num) force /*we need "force" because we want to force everything to numeric. if Stata can't convert it to numeric (e.g., "N/A" in the birthweight field) then it will set it to missing*/

histogram birthweight_num , width(100) 
...
/*Step 4. Summarize the data. For example, now we can look at birthweight by county, for example*/
...
gen charge_cost_diff=totalcharges-totalcosts

summ charge_cost_diff if regex(ccsr_full, "FRACTURE") 
summ totalcharges if regex(ccsr_full, "FRACTURE") 
summ totalcosts if regex(ccsr_full, "FRACTURE") 

tabstat charge_cost_diff if regex(ccsr_full, "FRACTURE") , by(ccsr_full)
...
gen died=.
replace died = 1 if patientdisposition=="Expired"
replace died = 0 if patientdisposition != "Expired" & patientdisposition != ""
tab died, m
...
use SPARCS_class, replace


use unadj_facility_means, replace
rename _margin _unadjusted_mortality

merge 1:1 _m1 using adj_facility_means
rename _margin _adjusted_mortality

list _m1 _unadjusted_mortality _adjusted_mortality

 gen _unadjusted_mortality_clean = round(_unadjusted_mortality*100, 0.1)
gen _adjusted_mortality_clean = round(_adjusted_mortality*100, 0.1)
gen change=round((_adjusted_mortality-_unadjusted_mortality)*100, 0.1)
list _m1 _unadjusted_mortality_clean _adjusted_mortality_clean  change
